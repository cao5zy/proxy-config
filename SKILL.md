...
Every micro-app requires `micro-app.yml` in its root directory. Select one package mode:
...
| `.env` | Optional runtime environment variables; source packages also pass it as build arguments | Service configuration, VITE_BASE_URL, etc. |
...
| `package_type` | No | `source` \| `image` \| `registry` | Defaults to `source`; `image` imports a prebuilt Docker archive; `registry` uses an existing registry image without source or Dockerfile |
...
| `image` | Registry only | `string` | Existing image reference such as `postgres:15.8-alpine`; `micro_proxy build` pulls it only when absent locally |
...
For official infrastructure images that have no application source, such as Postgres or Redis, use `package_type: registry` plus `image: postgres:15.8-alpine`. No Dockerfile, source checkout, or exported archive is required. `micro_proxy build` only runs `docker pull` if that exact reference is absent locally; deploy it with the same reference. Pin an exact version or digest in production instead of a mutable tag.
...
1. Running `micro_proxy build` after scanning configured application directories
...
└─────────────────┘                       └──────────────────┘
       ...
       ▼                                         ▼
┌─────────────────┐                     ┌──────────────────┐
│ Dockerfile or   │                     │   nginx.conf     │
│ image archive   │                     │ (auto-generated) │
└─────────────────┘                     └──────────────────┘
       │
       │ Read config
       ▼
...
- `Dockerfile` for source packages, or a `docker save` archive plus `package_type: image` / `image_archive` for image packages
...
| Can I edit `apps-config.yml` manually? | Not recommended. It is regenerated by `micro_proxy build`. Edit `micro-app.yml` or `micro-app.volumes.yml` instead. |
...
4. Guide deployment
   ├── Run `micro_proxy build` to scan and build/import the package
   ├── Deploy the selected immutable image, then run `micro_proxy start`
