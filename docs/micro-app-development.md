...
| 字段 | 是否必需 | 说明 |
|------|----------|------|
| `source` | ✅ | 宿主机路径（支持相对路径和绝对路径。相对路径对于 docker-compose 卷映射来说相对于 `docker-compose.yml` 所在目录，对于权限初始化脚本来说相对于微应用目录） |
| `target` | ✅ | 容器内路径 |
| `permissions` | ❌ | 权限配置对象 |
...
- **相对路径**：对于 docker-compose 卷映射，相对于 `docker-compose.yml` 所在目录；对于权限初始化脚本（`mkdir -p` + `chown`），相对于微应用目录（`micro-app.volumes.yml` 所在目录）
- **绝对路径**：使用完整路径，如 `/var/data`。权限初始化脚本直接使用此绝对路径
...
- 权限设置是在 `micro_proxy start` 部署时自动完成的：工具会生成包含 `mkdir -p` 和 `chown` 的脚本，在容器启动前执行。如果 `chown` 需要 root 权限，会自动尝试 `sudo`；如果都失败，会输出警告信息并提供手动执行的命令
